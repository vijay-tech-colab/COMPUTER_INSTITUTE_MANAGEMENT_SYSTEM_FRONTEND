...
// ─── Mock Data (Based on Course Schema) ──────────────────────
...
const coursesData = [
  {
    ...
    feeStructure: { totalFee: 45000, registrationAmount: 5000, installmentsAllowed: true },
    isActive: true,
  ...
  },
  {
    ...
    feeStructure: { totalFee: 25000, registrationAmount: 2500, installmentsAllowed: true },
    isActive: true,
  ...
  },
  {
    ...
    feeStructure: { totalFee: 32000, registrationAmount: 3000, installmentsAllowed: true },
    isActive: false,
  ...
  },
  {
    ...
    feeStructure: { totalFee: 18000, registrationAmount: 1500, installmentsAllowed: false },
    isActive: true,
    branch: "Pune Hub"
  },
]
...
function CourseStatCard({ title, value, icon: Icon, color, trend }: any) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-[10px] text-muted-foreground font-semibold flex items-center gap-1.5 uppercase tracking-wider">
            ...
            Course lifecycle and syllabus management
          </p>
        </div>
      ...
      <Card className={cn(
        ...
        <CardHeader className="px-5 py-3.5 flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-border/30 bg-muted/5">
          <div className="flex flex-col gap-0.5 w-full sm:w-auto">
            <CardTitle className="text-sm font-bold tracking-tight">Active Academic Programs</CardTitle>
            <CardDescription className="text-[9px] font-semibold uppercase tracking-[0.1em] opacity-70">Cataloging & Fee Calibration</CardDescription>
          </div>
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            ...
            <Button variant="outline" size="icon" className="h-7.5 w-7.5 rounded-lg shrink-0 border-border/40 hover:bg-primary/5 hover:text-primary">
            ...
            <thead>
              <tr className="border-b border-border/30 bg-muted/10">
                <th className="pl-6 pr-2 py-2 w-[40px]">
                  <Checkbox
                    checked={selectedCourses.length === coursesData.length}
                ...
                <th className="px-3 py-2 font-bold text-[9px] text-muted-foreground uppercase tracking-[0.1em]">Fee Metrics</th>
                <th className="px-3 py-2 font-bold text-[9px] text-muted-foreground uppercase tracking-[0.1em]">Duration</th>
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2.5">
                        <div className="h-8 w-8 rounded-lg bg-primary/5 border border-primary/10 flex items-center justify-center text-primary transition-transform group-hover:rotate-12">
                          <BookOpen className="h-3.5 w-3.5" />
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="font-semibold text-foreground leading-tight truncate">{course.name}</span>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[9px] text-muted-foreground/60 font-bold uppercase tracking-widest leading-none">
                              {course.code}
                            </span>
                            <span className="text-[8px] bg-muted/40 px-1 rounded opacity-50 font-bold border border-border/40">{course.branch}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-1 font-semibold text-foreground/90">
                          <Banknote className="h-2.5 w-2.5 text-emerald-500/60" />
                          {formatCurrency(course.feeStructure.totalFee)}
                        </div>
                        <div className="flex items-center gap-1.5 text-[9px] text-muted-foreground/60 font-bold uppercase tracking-tighter">
                          Registration: <span className="text-primary/60">{formatCurrency(course.feeStructure.registrationAmount)}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-1.5 font-bold text-foreground/70 text-[10px]">
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <div className="flex justify-center">
                        {course.isActive ? (
                          <Badge className="bg-emerald-500/10 text-emerald-600 border-0 text-[8px] font-bold uppercase tracking-[0.1em] px-2 py-0.5"><CheckCircle2 className="h-2 w-2 mr-1" /> Active</Badge>
                        ) : (
                          <Badge className="bg-red-500/10 text-red-600 border-0 text-[8px] font-bold uppercase tracking-[0.1em] px-2 py-0.5"><XCircle className="h-2 w-2 mr-1" /> Inactive</Badge>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-2.5 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-[160px] rounded-xl border-border/40 shadow-xl backdrop-blur-lg bg-card/90">
                          <DropdownMenuLabel className="text-[8px] font-bold uppercase tracking-[0.2em] text-muted-foreground px-3 py-2">Academy Ops</DropdownMenuLabel>
                          <DropdownMenuItem className="gap-2 px-3 font-semibold text-[11px] py-1.5 focus:bg-primary focus:text-white cursor-pointer">
                            <Info className="h-3 w-3" /> View Syllabus
                          </DropdownMenuItem>
                          <DropdownMenuItem className="gap-2 px-3 font-semibold text-[11px] py-1.5 focus:bg-primary focus:text-white cursor-pointer">
                            <Edit className="h-3 w-3" /> Edit Program
                          </DropdownMenuItem>
                          ...
                          <DropdownMenuItem className="gap-2 px-3 font-semibold text-[11px] py-1.5 focus:bg-red-500 focus:text-white text-red-500 cursor-pointer">
                            <Trash2 className="h-3 w-3" /> Archive Course
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </td>
