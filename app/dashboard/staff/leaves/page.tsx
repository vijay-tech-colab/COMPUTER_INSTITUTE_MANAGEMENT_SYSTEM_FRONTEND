...
const leavesData = [
  ...
  },
  {
    ...
    type: "Casual Leave",
    icon: Briefcase,
    period: "25 Apr - 25 Apr",
    duration: "1 Day",
    status: "Pending",
    reason: "Personal family meeting",
    initials: "SS"
  },
  {
    id: "LEV-903",
...
function LeaveStatCard({ title, value, icon: Icon, color, trend }: any) {
...
export default function StaffLeavesPage() {
  ...
  const toggleSelectAll = () => {
    if (selectedLeaves.length === leavesData.length) {
      setSelectedLeaves([])
    } else {
      setSelectedLeaves(leavesData.map(l => l.id))
    }
  }
  ...
  const getStatusBadge = (status: string) => {
    ...
    switch (status) {
      case "Approved":
        return <Badge className={cn(baseClass, "bg-emerald-500/10 text-emerald-600")}><CheckCircle2 className="h-2.5 w-2.5" /> Authorized</Badge>
      case "Pending":
        return <Badge className={cn(baseClass, "bg-amber-500/10 text-amber-600")}><Clock className="h-2.5 w-2.5" /> Reviewing</Badge>
      case "Rejected":
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-[10px] text-muted-foreground font-semibold flex items-center gap-1.5 uppercase tracking-wider">
            ...
            Absence tracking and resource availability
          </p>
        </div>
      ...
      <Card className={cn(
        ...
        <CardHeader className="px-5 py-3.5 flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-border/30 bg-muted/5">
          <div className="flex flex-col gap-0.5 w-full sm:w-auto">
            <CardTitle className="text-sm font-bold tracking-tight">Staff Absence Ledger</CardTitle>
            <CardDescription className="text-[9px] font-semibold uppercase tracking-[0.1em] opacity-70">Cataloging workforce availability and out-of-office logs</CardDescription>
          </div>
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            <div className="relative group flex-1 sm:min-w-[280px]">
              ...
              <Input
                placeholder="Find staff name or leave type..."
                value={searchTerm}
            ...
            <thead>
              <tr className="border-b border-border/30 bg-muted/10">
                <th className="pl-6 pr-2 py-2 w-[40px]">
                  <Checkbox 
                    checked={selectedLeaves.length === leavesData.length}
                ...
                <th className="px-3 py-2 font-bold text-[9px] text-muted-foreground uppercase tracking-[0.1em]">Classification</th>
                <th className="px-3 py-2 font-bold text-[9px] text-muted-foreground uppercase tracking-[0.1em]">Timeline</th>
                <th className="px-3 py-2 font-bold text-[9px] text-muted-foreground uppercase tracking-[0.1em]">Outlay</th>
                ...
                {leavesData.map((leave, idx) => (
                  <motion.tr
                    ...
                    className={cn(
                      ...
                      selectedLeaves.includes(leave.id) && "bg-primary/5"
                    )}
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2.5">
                         <Avatar className="h-8 w-8 ring-1 ring-border/40 group-hover:ring-primary/20 shadow-sm transition-all duration-300">
                           <AvatarFallback className="bg-primary/5 text-primary text-[9px] font-bold">{leave.initials}</AvatarFallback>
                         </Avatar>
                         <div className="flex flex-col min-w-0">
                            <span className="font-bold text-foreground leading-tight truncate">{leave.staffName}</span>
                            <span className="text-[9px] text-muted-foreground/60 font-bold uppercase tracking-widest mt-0.5">{leave.id}</span>
                         </div>
                      </div>
                    </td>
                    <td className="px-3 py-2.5">
                       <div className="flex items-center gap-2">
                          <div className="h-7 w-7 rounded-lg bg-muted/30 border border-border/20 flex items-center justify-center text-muted-foreground group-hover:scale-105 transition-transform">
                             <leave.icon className="h-3.5 w-3.5" />
                          </div>
                          <span className="text-[10px] font-semibold text-foreground/80 tracking-tight">{leave.type}</span>
                       </div>
                    </td>
                    <td className="px-3 py-2.5">
                       <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[9px] text-muted-foreground/60 font-bold uppercase tracking-widest truncate max-w-[120px]">
                             {leave.reason}
                          </span>
                       </div>
                    </td>
                    <td className="px-3 py-2.5">
                       <span className="font-bold text-foreground tracking-tight">{leave.duration}</span>
                    </td>
                    ...
                    <td className="px-6 py-2.5 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-[160px] rounded-xl border-border/40 shadow-xl backdrop-blur-lg bg-card/90">
                          <DropdownMenuLabel className="text-[8px] font-bold uppercase tracking-[0.2em] text-muted-foreground px-3 py-2">Leave Ops</DropdownMenuLabel>
                          <DropdownMenuItem className="gap-2 px-3 font-semibold text-[11px] py-1.5 focus:bg-emerald-500 focus:text-white cursor-pointer transition-colors">
                            <CheckCircle2 className="h-3 w-3" /> Approve Now
                          </DropdownMenuItem>
                          <DropdownMenuItem className="gap-2 px-3 font-semibold text-[11px] py-1.5 focus:bg-red-500 focus:text-white cursor-pointer transition-colors">
                            <XCircle className="h-3 w-3" /> Decline Request
                          </DropdownMenuItem>
